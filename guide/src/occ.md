...
As an example, consider systems like Google Cloud IAM, where the shared resource
is an **IAM Policy** applied to a resource (like a Project, Bucket, or Service).
To implement OCC, systems typically use a version number or an `etag` (entity
tag) field on the resource struct.
...
1. Process **B** reads the *same* current state.

1. Process **A** modifies its copy and writes it back to the server.

1. Process **B** modifies its copy and writes it back to the server.
...
entity is modified. In many systems (like IAM), this is done using an `etag`.
...
1. If the server finds that the stored `etag` does **not** match the `etag` you
   sent (meaning someone else modified the resource since you read it), the
   write operation fails with an `ABORTED` or `FAILED_PRECONDITION` error.
