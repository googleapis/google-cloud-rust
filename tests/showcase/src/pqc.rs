...
pub async fn run() -> Result<()> {
    ...
    // SAFETY: Single-threaded integration test process with no concurrent env access.
    unsafe {
...
async fn wait_for_ca_cert(child: &mut tokio::process::Child) -> Result<()> {
    ...
    // Wait until Showcase logs that it wrote the CA certificate.
    tokio::time::timeout(Duration::from_secs(30), async {
        ...
        Err(Error::msg(
            "showcase process closed stdout before writing CA certificate",
        ))
    })
    .await
    .map_err(|_| {
        Error::msg("timed out waiting for showcase to write CA certificate within 30s")
    })??;

    // Spawn background task to drain remaining showcase logs.
