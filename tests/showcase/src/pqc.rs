...
pub async fn run() -> Result<()> {
    ...
    for _ in 0..50 {
        if let Ok(pem) = std::fs::read(&ca_cert_path)
            && !pem.is_empty()
        {
            // SAFETY: `std::env::set_var` is unsafe in Rust because concurrent reads or writes
            // to the environment in a multi-threaded process cause a data race in POSIX (`setenv`/`getenv`).
            // This is safe here because this integration test (`tests/pqc.rs`) runs in its own isolated
            // OS process with a single test thread, so no concurrent environment access can occur.
            // We set `SSL_CERT_FILE` globally because the client does not yet expose a builder option
            // to configure custom root CA certificates
            unsafe {
