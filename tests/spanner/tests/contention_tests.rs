...
//! High-concurrency benchmarks and stress tests for Spanner channel pooling.
//!
//! These tests simulate connection-level queuing, contention, and processing limits under
//! concurrent point-select and read-write workloads.
//!
//! Because these tests run multi-worker workloads with real-time delays and spiky load patterns
//! (some lasting up to 2 minutes), they are disabled by default with `#[ignore]` so standard
//! `cargo test` execution remains fast in local development and CI.
//!
//! To run these benchmarks manually:
//! ```bash
//! cargo test -p integration-tests-spanner --test contention_tests -- --ignored --nocapture
//! ```
...
use spanner_grpc_mock::google::spanner::v1::spanner_server::{
    Spanner as SpannerServerTrait, SpannerServer as SpannerGrpcServer,
};
use spanner_grpc_mock::google::spanner::v1::struct_type::Field;
use spanner_grpc_mock::google::spanner::v1::transaction_selector::Selector;
use spanner_grpc_mock::google::spanner::v1::{
    BatchCreateSessionsRequest, BatchCreateSessionsResponse, BatchWriteRequest, BatchWriteResponse,
    BeginTransactionRequest, CacheUpdate, CommitRequest, CommitResponse, CreateSessionRequest,
    DeleteSessionRequest, ExecuteBatchDmlRequest, ExecuteBatchDmlResponse, ExecuteSqlRequest,
    FetchCacheUpdateRequest, GetSessionRequest, ListSessionsRequest, ListSessionsResponse,
    PartialResultSet, PartitionQueryRequest, PartitionReadRequest, PartitionResponse, ReadRequest,
    ResultSet, ResultSetMetadata, ResultSetStats, RollbackRequest, Session, StructType,
...
/// Simulates server connection-level queuing and processing contention.
///
...
pub(crate) struct ChannelContentionManager {
    channels: Arc<RwLock<HashMap<SocketAddr, Arc<Semaphore>>>>,
...
impl ChannelContentionManager {
    ...
    pub(crate) fn register_channel(&self, client_address: SocketAddr) {
        let mut registry = self.channels.write().expect("lock poisoned");
        registry
            .entry(client_address)
            .or_insert_with(|| Arc::new(Semaphore::new(self.max_parallel_per_channel)));
    }

    pub(crate) fn connected_channel_count(&self) -> usize {
        self.channels.read().expect("lock poisoned").len()
    }
    ...
    pub(crate) fn reset_counters(&self) {
        self.total_queued_requests.store(0, Ordering::Relaxed);
        self.total_completed_requests.store(0, Ordering::Relaxed);
    }
}

#[derive(Clone)]
struct ContentionSpannerServer {
    contention_manager: ChannelContentionManager,
}

#[async_trait]
impl SpannerServerTrait for ContentionSpannerServer {
    type ExecuteStreamingSqlStream = ReceiverStream<Result<PartialResultSet, Status>>;
    type StreamingReadStream = ReceiverStream<Result<PartialResultSet, Status>>;
    type BatchWriteStream = ReceiverStream<Result<BatchWriteResponse, Status>>;
    type FetchCacheUpdateStream = ReceiverStream<Result<CacheUpdate, Status>>;
    ...
    async fn batch_create_sessions(
        &self,
        _request: Request<BatchCreateSessionsRequest>,
    ) -> Result<Response<BatchCreateSessionsResponse>, Status> {
        Err(Status::unimplemented(
            "not implemented for contention benchmark",
        ))
    }
    ...
    async fn execute_sql(
        ...
        self.contention_manager
            .execute_with_queue(client_address)
            .await;
    ...
    async fn execute_streaming_sql(
        &self,
        request: Request<ExecuteSqlRequest>,
    ) -> Result<Response<Self::ExecuteStreamingSqlStream>, Status> {
        let client_address = request
            .remote_addr()
            .expect("remote client address must be present on TCP transport");
        let is_begin = request
        ...
        let contention_manager = self.contention_manager.clone();
        spawn(async move {
    ...
    async fn execute_batch_dml(
        &self,
        request: Request<ExecuteBatchDmlRequest>,
        ...
        self.contention_manager
            .execute_with_queue(client_address)
            .await;
        Ok(Response::new(ExecuteBatchDmlResponse::default()))
    }

    async fn read(&self, _request: Request<ReadRequest>) -> Result<Response<ResultSet>, Status> {
        Err(Status::unimplemented(
            "not implemented for contention benchmark",
        ))
    }
    ...
    async fn rollback(&self, _request: Request<RollbackRequest>) -> Result<Response<()>, Status> {
        Ok(Response::new(()))
    }
    ...
    async fn partition_read(
        &self,
        _request: Request<PartitionReadRequest>,
    ) -> Result<Response<PartitionResponse>, Status> {
        Err(Status::unimplemented(
            "not implemented for contention benchmark",
        ))
    ...
    ) -> Result<Response<Self::BatchWriteStream>, Status> {
        Err(Status::unimplemented(
            "not implemented for contention benchmark",
        ))
    }
    ...
    async fn fetch_cache_update(
        &self,
        _request: Request<FetchCacheUpdateRequest>,
    ) -> Result<Response<Self::FetchCacheUpdateStream>, Status> {
        Err(Status::unimplemented(
            "not implemented for contention benchmark",
        ))
    }
...
async fn start_contention_server(
    ...
    let listener = TcpListener::bind("127.0.0.1:0")
        .await
        .expect("mock TCP listener must bind");
    let addr = listener
        .local_addr()
        .expect("local address must be available");

    let contention_manager_clone = contention_manager.clone();
    let server = spawn(async move {
        let stream = TcpListenerStream::new(listener);
        let stream = stream.map(move |result| {
            if let Some(peer_addr) = result
                .as_ref()
                .ok()
                .and_then(|tcp_stream| tcp_stream.peer_addr().ok())
            {
                contention_manager_clone.register_channel(peer_addr);
            }
            result
        });

        let _ = Server::builder()
            .add_service(SpannerGrpcServer::new(ContentionSpannerServer {
                contention_manager,
            }))
...
impl BenchmarkRunMetrics {
    fn calculate(queued_operations: usize, mut latencies: Vec<Duration>) -> Self {
        ...
        let operations_over_15ms = latencies
            .iter()
            .filter(|&&latency| latency >= Duration::from_millis(15))
            .count();
...
#[tokio_test]
#[serial]
#[ignore = "benchmark testing dynamic pool scale-up under queuing; run with -- --ignored"]
async fn dynamic_channel_pool_scales_up_and_reduces_contention() -> Result<()> {
    ...
    // Warm up initial channels concurrently so physical TCP connections are established.
    ...
    while let Some(join_result) = init_set.join_next().await {
        join_result.expect("init task join failed")?;
    }

    assert_eq!(
        contention_manager.connected_channel_count(),
        4,
    ...
    for _ in 0..total_workers {
        let client_clone = database_client.clone();
        let stopped = Arc::clone(&is_stopped);
    ...
    loop {
        final_channels = contention_manager.connected_channel_count();
    ...
    while let Some(join_result) = join_set.join_next().await {
        let worker_latencies = join_result.expect("worker task join should succeed")?;
    ...
    for _ in 0..10 {
        ...
        while let Some(join_result) = wave_set.join_next().await {
            post_scale_latencies.push(join_result.expect("worker task join should succeed")?);
        }
    }
    ...
    let post_metrics = BenchmarkRunMetrics::calculate(0, post_scale_latencies);
    let burst_metrics =
    ...
    eprintln!(
        ...
        post_metrics.total_operations,
        post_metrics.p50_latency,
        post_metrics.p95_latency,
        post_metrics.p99_latency
    );

    assert!(
        post_metrics.p50_latency < burst_metrics.p50_latency,
        "post-scale p50 latency ({:?}) should be lower than contended burst p50 ({:?})",
        post_metrics.p50_latency,
        burst_metrics.p50_latency
    );
...
#[tokio_test]
#[serial]
#[ignore = "benchmark testing fixed static pool size under queuing; run with -- --ignored"]
async fn static_channel_pool_remains_at_fixed_size_under_contention() -> Result<()> {
    let contention_manager = ChannelContentionManager::new(4, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
    ...
    warm_up_database_client(&database_client, &contention_manager, 4).await?;
    ...
    while let Some(join_result) = join_set.join_next().await {
        join_result.expect("worker task join should succeed")?;
    }
...
async fn warm_up_database_client(
...
#[tokio_test]
#[serial]
#[ignore = "spiky workload benchmark testing dynamic pool scaling under queuing; run with -- --ignored"]
async fn replication_spiky_workload_database_client_under_default_dynamic_pool() -> Result<()> {
    ...
    let calm_start_queued = contention_manager.total_queued_count();
    let calm_latencies =
        run_database_client_point_select_workload(&database_client, 2, Duration::from_millis(300))
            .await?;

    let calm_queued_count = contention_manager.total_queued_count() - calm_start_queued;
    ...
    // Phase 2: Spiky Burst State (16 concurrent workers for 800ms)
    // With 4 channels and 2 permits per channel, 16 workers creates 4 requests per channel (> 2 permits).
    let burst_start_queued = contention_manager.total_queued_count();
    let burst_latencies =
        run_database_client_point_select_workload(&database_client, 16, Duration::from_millis(800))
            .await?;

    let burst_queued_count = contention_manager.total_queued_count() - burst_start_queued;
    let final_channels = contention_manager.connected_channel_count();
...
#[tokio_test]
#[serial]
#[ignore = "spiky workload benchmark testing 64-channel static pool under queuing; run with -- --ignored"]
async fn replication_spiky_workload_database_client_under_64_channel_static_pool() -> Result<()> {
    let contention_manager = ChannelContentionManager::new(2, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
    ...
    assert!(
        contention_manager.connected_channel_count() <= 64,
        "static 64-channel pool connections must not exceed 64"
    );
...
#[tokio_test]
#[serial]
#[ignore = "burst benchmark testing 4s cooldown throttling behavior under queuing; run with -- --ignored"]
async fn replication_dynamic_pool_4s_cooldown_bottleneck_under_short_burst() -> Result<()> {
    let contention_manager = ChannelContentionManager::new(2, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
    ...
    let mut burst_latencies = Vec::new();
    while let Some(join_result) = join_set.join_next().await {
        burst_latencies.extend(join_result.expect("worker task join should succeed")?);
    }

    let final_channels = contention_manager.connected_channel_count();
    let burst_queued_count = contention_manager.total_queued_count() - burst_start_queued;
    let metrics = BenchmarkRunMetrics::calculate(burst_queued_count, burst_latencies);
...
#[tokio_test]
#[serial]
#[ignore = "burst benchmark testing fast cooldown recovery under queuing; run with -- --ignored"]
async fn replication_dynamic_pool_fast_cooldown_recovers_under_burst() -> Result<()> {
    let contention_manager = ChannelContentionManager::new(2, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
...
#[tokio_test]
#[serial]
#[ignore = "DatabaseClient benchmark testing dynamic scaling with fast cooldown under queuing; run with -- --ignored"]
async fn replication_database_client_scales_dynamically_with_fast_cooldown() -> Result<()> {
    let contention_manager = ChannelContentionManager::new(2, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
...
async fn warm_up_select_update(
    database_client: &DatabaseClient,
    contention_manager: &ChannelContentionManager,
    target_channels: usize,
) -> Result<()> {
    for _ in 0..100 {
        ...
        runner
            .run(|transaction: ReadWriteTransaction| async move {
                ...
                let mut result_set = transaction.execute_query(statement).await?;
                assert!(
                    result_set.next().await.transpose()?.is_some(),
                    "expected warmup row to be returned"
                );
                drop(result_set);
...
#[tokio_test]
#[serial]
#[ignore = "read-write transaction benchmark testing default dynamic pool under queuing; run with -- --ignored"]
async fn replication_select_update_database_client_under_default_dynamic_pool() -> Result<()> {
    let contention_manager = ChannelContentionManager::new(2, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
    ...
    warm_up_select_update(&database_client, &contention_manager, 4).await?;
...
#[tokio_test]
#[serial]
#[ignore = "read-write transaction benchmark testing 64-channel static pool under queuing; run with -- --ignored"]
async fn replication_select_update_database_client_under_64_channel_static_pool() -> Result<()> {
    let contention_manager = ChannelContentionManager::new(2, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
    ...
    assert!(
        contention_manager.connected_channel_count() <= 64,
        "static pool must not exceed 64 channels"
    );
...
#[tokio_test]
#[serial]
#[ignore = "read-write transaction benchmark testing fast cooldown scaling under queuing; run with -- --ignored"]
...
#[tokio_test]
#[serial]
#[ignore = "2-minute multi-cycle spiky read-write benchmark for dynamic pool; run with -- --ignored"]
async fn replication_select_update_database_client_2min_spiky_default_dynamic_pool() -> Result<()> {
    let contention_manager = ChannelContentionManager::new(2, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
    ...
    for cycle in 1..=6 {
        ...
        let calm_channels = contention_manager.connected_channel_count();
        ...
        let burst_channels = contention_manager.connected_channel_count();
...
#[tokio_test]
#[serial]
#[ignore = "2-minute multi-cycle spiky read-write benchmark for 64-channel static pool; run with -- --ignored"]
async fn replication_select_update_database_client_2min_spiky_64_channel_static_pool() -> Result<()>
{
    let contention_manager = ChannelContentionManager::new(2, 1500, 500);
    let (address, _server) = start_contention_server(contention_manager.clone()).await;
    ...
    assert!(
        contention_manager.connected_channel_count() <= 64,
        "static channel pool count must not exceed 64"
    );
