...
async fn test_bidi_write_resume_append(client: &Storage, bucket: &str) -> anyhow::Result<()> {
    ...
    tracing::info!("closed stream 1, persisted_size: {}", persisted_size);
    ...
    tracing::info!(
        "reopened stream, persisted_size: {}",
        writer2.persisted_size()
    );
...
async fn test_bidi_write_drop_stream(client: &Storage, bucket: &str) -> anyhow::Result<()> {
    tracing::info!("run test_bidi_write_drop_stream");
