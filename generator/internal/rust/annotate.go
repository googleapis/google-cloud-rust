...
func (c *codec) annotateOneOf(oneof *api.OneOf, message *api.Message, state *api.APIState, sourceSpecificationPackageName string, usedNames map[string]bool) {
	...
	if _, ok := usedNames[enumName]; ok {
		// Very rarely (once in all of the googleapis) a nested message and
		// a oneof have conflicting names. We append `Oneof` to the
		// generated union type.
		enumName = enumName + "OneOf"
	}
