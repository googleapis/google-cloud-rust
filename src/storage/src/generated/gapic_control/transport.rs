...
impl super::stub::StorageControl for StorageControl {
    ...
    async fn view_object_full_context(
        ...
        type TR = crate::google::storage::control::v2::ObjectFullContext;
        if let Some(recorder) = gaxi::observability::RequestRecorder::current() {
            let attributes = gaxi::observability::ClientRequestAttributes::default()
                .set_rpc_method("google.storage.control.v2.StorageControl/ViewObjectFullContext");
            let resource_name = (|| {
                Some(format!(
                    "//storage.googleapis.com/{}",
                    Some(&req).map(|m| &m.name).map(|s| s.as_str())?,
        ...
        self.inner
            ...
            .await
            .and_then(gaxi::grpc::to_gax_response::<TR, crate::model::ObjectFullContext>)
    }
