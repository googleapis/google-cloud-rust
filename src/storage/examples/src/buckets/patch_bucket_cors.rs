...
// [START storage_patch_cors_configuration]
...
pub async fn sample(client: &StorageControl, bucket_id: &str) -> anyhow::Result<()> {
    ...
    if let Some(rule) = cors.iter_mut().find(|c| c.max_age_seconds == 3600) {
        rule.method.push("PUT".to_string());
    }
    ...
    let bucket = client
        ...
        .send()
        .await?;
    println!("successfully patched bucket CORS for {bucket_id}: {bucket:?}");
    Ok(())
}
// [END storage_patch_cors_configuration]
