...
pub async fn sample(client: &StorageControl, bucket_id: &str, kms_key: &str) -> anyhow::Result<()> {
    let bucket = client
        .get_bucket()
    ...
    let bucket = client
        .update_bucket()
        .set_bucket(bucket.set_encryption(Encryption::new().set_default_kms_key(kms_key)))
