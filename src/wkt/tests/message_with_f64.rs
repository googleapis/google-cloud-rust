...
mod tests {
    ...
    #[test_case(r#"{"singular": 0.8}"#, MessageWithF64::new().set_singular(0.8_f64))]
    #[test_case(r#"{"optional": -0.25}"#, MessageWithF64::new().set_optional(-0.25_f64))]
    #[test_case(r#"{"repeated": [0.5, 1, "2.5"]}"#, MessageWithF64::new().set_repeated([0.5_f64, 1.0, 2.5]))]
    #[test_case(r#"{"map": {"a": 0.5, "b": 1.5e1}}"#, MessageWithF64::new().set_map([("a", 0.5_f64), ("b", 15.0)]))]
    fn test_de_json_text(input: &str, want: MessageWithF64) -> Result {
        let got = serde_json::from_str::<MessageWithF64>(input)?;
        assert_eq!(got, want);
        Ok(())
    }
