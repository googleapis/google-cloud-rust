...
pub trait RequestOptionsBuilder: internal::RequestBuilder {
    ...
    ///
    /// Callers who want to configure respective system headers should use the dedicated methods instead:
    /// * `user-agent`: Use [`with_user_agent`](Self::with_user_agent) (or globally via `ClientBuilder`).
    /// * `x-goog-user-project`: Use [`with_quota_project`](Self::with_quota_project).
    /// * `authorization` and `x-goog-api-key`: Configure globally via `ClientBuilder::with_credentials`.
    ///
    /// Note that `x-goog-api-client` and `x-goog-request-params` are dynamically auto-generated by the SDK
    /// for telemetry and routing, and cannot be customized by callers.
    ///
    /// ### Repeated Headers
    ///
