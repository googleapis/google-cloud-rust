...
use super::ext::{PollerExt, PollerOptions};
...
where
    ...
    QF: std::future::Future<Output = Result<Operation<ResponseType, MetadataType>>>
        ...
        + 'static,
{
    new_poller(polling_error_policy, polling_backoff_policy, start, query).with_options(options)
}
...
where
    ...
    QF: std::future::Future<Output = Result<Operation<Empty, MetadataType>>> + Send + 'static,
{
    new_unit_response_poller(polling_error_policy, polling_backoff_policy, start, query)
...
where
    ...
    QF: std::future::Future<Output = Result<Operation<ResponseType, Empty>>> + Send + 'static,
{
    new_unit_metadata_poller(polling_error_policy, polling_backoff_policy, start, query)
        .with_options(options)
}
...
where
    ...
    QF: std::future::Future<Output = Result<Operation<Empty, Empty>>> + Send + 'static,
{
    new_unit_poller(polling_error_policy, polling_backoff_policy, start, query)
        .with_options(options)
}
