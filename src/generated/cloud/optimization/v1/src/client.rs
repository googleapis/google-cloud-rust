...
impl FleetRouting {
    ...
    /// ```
    /// # use gax::Result;
    /// # use google_cloud_optimization_v1::model;
    /// async fn wait(
    ///     mut poller: impl lro::Poller<model::BatchOptimizeToursResponse, model::AsyncModelMetadata>
    /// ) -> Result<model::BatchOptimizeToursResponse> {
    ...
    /// field contains the final response, this will be a [crate::model::BatchOptimizeToursResponse] (as
    /// an [Any]), or the error (as a `Status`).
    ///
    /// If the `done` field is `false`, the operation has not completed.  The
    /// operation may also include a [crate::model::AsyncModelMetadata] value in the `metadata`
    ...
    ///
    /// [send()]: crate::builders::fleet_routing::BatchOptimizeTours::send
    /// [poller()]: crate::builders::fleet_routing::BatchOptimizeTours::poller
