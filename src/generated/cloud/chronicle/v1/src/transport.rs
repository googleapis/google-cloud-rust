...
impl super::stub::DataAccessControlService for DataAccessControlService {
    ...
    async fn update_data_access_label(
        ...
        let options = {
            let resource_name =
                Option::<&String>::None.or(req.data_access_label.as_ref().map(|s| &s.name));
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//chronicle.googleapis.com/{}", rn);
                gax::options::internal::set_resource_name(options, full_resource_name)
            } else {
    ...
    async fn update_data_access_scope(
        ...
        let options = {
            let resource_name =
                Option::<&String>::None.or(req.data_access_scope.as_ref().map(|s| &s.name));
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//chronicle.googleapis.com/{}", rn);
                gax::options::internal::set_resource_name(options, full_resource_name)
            } else {
...
impl super::stub::RuleService for RuleService {
    ...
    async fn update_rule(
        ...
        #[cfg(google_cloud_unstable_tracing)]
        let options = {
            let resource_name = Option::<&String>::None.or(req.rule.as_ref().map(|s| &s.scope));
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//chronicle.googleapis.com/{}", rn);
                gax::options::internal::set_resource_name(options, full_resource_name)
            } else {
