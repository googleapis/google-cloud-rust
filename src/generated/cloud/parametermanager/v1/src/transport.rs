...
impl super::stub::ParameterManager for ParameterManager {
    async fn list_parameters(
        ...
        let (builder, method, _path_template, _resource_name) = None
            .or_else(|| {
                ...
                let path = format!("/v1/{}/parameters", var_parent,);
                let path_template = "/v1/{parent}/parameters";

                let resource_name = format!("//parametermanager.googleapis.com/{}", var_parent,);
    ...
    async fn get_parameter(
        ...
        let (builder, method, _path_template, _resource_name) = None
            .or_else(|| {
                ...
                let resource_name = format!("//parametermanager.googleapis.com/{}", var_name,);
                let builder = self.inner.builder(Method::GET, path);
                let builder = Ok(builder);
                Some(builder.map(|b| (b, Method::GET, path_template, resource_name)))
            })
    ...
    async fn update_parameter(
        ...
        let (builder, method, _path_template) = None
            .or_else(|| {
                let var_parameter_name = try_match(
                    Some(&req)
                ...
                let path = format!("/v1/{}", var_parameter_name,);
    ...
    async fn render_parameter_version(
        ...
        let (builder, method, _path_template, _resource_name) = None
            .or_else(|| {
                ...
                let path = format!("/v1/{}:render", var_name,);
    ...
    async fn update_parameter_version(
        ...
        let (builder, method, _path_template) = None
            .or_else(|| {
                let var_parameter_version_name = try_match(
                    Some(&req)
                ...
                let path = format!("/v1/{}", var_parameter_version_name,);
    ...
    async fn get_template(
        ...
        if let Some(recorder) = gaxi::observability::RequestRecorder::current() {
            recorder.on_client_request(
                gaxi::observability::ClientRequestAttributes::default()
                    .set_rpc_method("google.cloud.parametermanager.v1.ParameterManager/GetTemplate")
                    .set_url_template(_path_template)
                    .set_resource_name(_resource_name),
            );
        }
    ...
    async fn create_template_version(
        ...
        let (builder, method, _path_template, _resource_name) = None
            .or_else(|| {
                ...
                let resource_name = format!("//parametermanager.googleapis.com/{}", var_parent,);
                let builder = self.inner.builder(Method::POST, path);
                let builder = builder.query(&[("templateVersionId", &req.template_version_id)]);
                let builder = builder.query(&[("requestId", &req.request_id)]);
                let builder = Ok(builder);
                Some(builder.map(|b| (b, Method::POST, path_template, resource_name)))
            })
        ...
        let body = gaxi::http::handle_empty(req.template_version, &method);
        self.inner.execute(builder, body, options).await
    }
    ...
    async fn update_template_version(
        ...
        let (builder, method, _path_template) = None
            .or_else(|| {
                let var_template_version_name = try_match(
                    Some(&req)
                        .and_then(|m| m.template_version.as_ref())
                        .map(|m| &m.name)
                        .map(|s| s.as_str()),
                    &[
                        Segment::Literal("projects/"),
                ...
                let path = format!("/v1/{}", var_template_version_name,);
                let path_template = "/v1/{template_version.name}";
    ...
    async fn render_template_version(
        ...
        let (builder, method, _path_template, _resource_name) = None
            .or_else(|| {
                ...
                let resource_name = format!("//parametermanager.googleapis.com/{}", var_name,);
                let builder = self.inner.builder(Method::GET, path);
                let builder = builder.query(&[("parameterVersion", &req.parameter_version)]);
                let builder = Ok(builder);
                Some(builder.map(|b| (b, Method::GET, path_template, resource_name)))
            })
            .ok_or_else(|| {
