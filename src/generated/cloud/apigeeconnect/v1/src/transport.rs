...
impl super::stub::ConnectionService for ConnectionService {
    async fn list_connections(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}/connections",
                try_match(Some(&req).map(|m| &m.parent).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/endpoints/"), Segment::SingleWildcard])?,
            );
        ...
        .ok_or_else(|| {
            let mut paths = Vec::new();
            {
                ...
                let builder = builder.maybe_add(
                    ...
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/endpoints/"), Segment::SingleWildcard],
                    "parent",
                    "projects/*/endpoints/*");
        ...
        let options = {
            ...
            if let Some(rn) = resource_name {
                let full_resource_name = format!("//apigeeconnect.googleapis.com/{}", rn);
