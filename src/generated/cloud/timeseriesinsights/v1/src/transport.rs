...
impl super::stub::TimeseriesInsightsController for TimeseriesInsightsController {
    ...
    async fn delete_data_set(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}",
                try_match(Some(&req).map(|m| &m.name).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/datasets/"), Segment::SingleWildcard])?,
            );
            let path_template = "/v1/{name}";
        ...
        .ok_or_else(|| {
            ...
            }
            {
                ...
                let builder = builder.maybe_add(
                    Some(&req).map(|m| &m.name).map(|s| s.as_str()),
                    &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/datasets/"), Segment::SingleWildcard],
                    "name",
                    "projects/*/datasets/*");
                paths.push(builder.build());
            }
    ...
    async fn append_events(
        ...
        .or_else(|| {
            let path = format!(
                "/v1/{}:appendEvents",
                try_match(Some(&req).map(|m| &m.dataset).map(|s| s.as_str()), &[Segment::Literal("projects/"), Segment::SingleWildcard, Segment::Literal("/datasets/"), Segment::SingleWildcard])?,
            );
            let path_template = "/v1/{dataset}:appendEvents";
