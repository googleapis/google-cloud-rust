...
/// [with_credentials()]: super::builder::iam_credentials::ClientBuilder::with_credentials
...
/// create one and reuse it. You do not need to wrap `IAMCredentials` in
...
impl IAMCredentials {
    ...
    /// ```
    /// # use google_cloud_iam_credentials_v1::client::IAMCredentials;
    /// use google_cloud_iam_credentials_v1::Result;
    /// async fn sample(
    ...
    ///    client: &IAMCredentials
    /// ) -> Result<()> {
    ///     let response = client.generate_id_token()
    ///         /* set fields */
    ///         .send().await?;
    ///     println!("response {:?}", response);
    ///     Ok(())
