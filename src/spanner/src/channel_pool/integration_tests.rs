...
async fn wait_for_in_flight(spanner: &Spanner, expected: u32, timeout_duration: Duration) {
    ...
    let effective_timeout = timeout_duration.max(Duration::from_secs(2));
...
fn setup_chunk(first: bool, last: bool) -> mock_v1::PartialResultSet {
...
async fn single_use_query_in_flight_lifecycle() -> Result<()> {
    ...
    assert!(
        ...
        "Stream must return first row"
    );

    assert!(
        result_set.next().await.transpose()?.is_none(),
...
#[tokio_test_no_panics]
async fn single_use_read_in_flight_lifecycle() -> Result<()> {
    let mut mock = create_session_mock();
    ...
    assert!(
        result_set.next().await.transpose()?.is_some(),
        "Read stream must return first row"
    );

    assert!(
        result_set.next().await.transpose()?.is_none(),
        "Read stream must reach EOF"
    );
...
#[tokio_test_no_panics]
async fn streaming_query_early_drop_releases_in_flight_immediately() -> Result<()> {
    let mut mock = create_session_mock();
    mock.expect_execute_streaming_sql().once().returning(|_| {
        Ok(Response::from(adapt([
            Ok(setup_chunk(true, false)),
            Ok(setup_chunk(false, false)),
            Ok(setup_chunk(false, true)),
        ])))
    });
...
#[tokio_test_no_panics]
async fn streaming_query_initial_error_releases_in_flight_and_applies_penalty() -> Result<()> {
    let mut mock = create_session_mock();
    mock.expect_execute_streaming_sql()
        .once()
...
#[tokio_test_no_panics]
async fn multi_use_read_only_transaction_pins_same_channel_inline_begin() -> Result<()> {
    let mut mock = create_session_mock();
    ...
    let mut result_set1 = transaction
        .execute_query(Statement::builder("SELECT 1").build())
        .await?;
    while let Some(row) = result_set1.next().await {
        let _ = row?;
    }
    ...
    let mut result_set2 = transaction
        ...
        .await?;
    while let Some(row) = result_set2.next().await {
        let _ = row?;
    }
...
#[tokio_test_no_panics]
async fn multi_use_read_only_transaction_pins_same_channel_explicit_begin() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
...
#[tokio_test_no_panics]
async fn multi_use_read_only_transaction_parallel_initial_queries_inline_begin() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
...
#[tokio_test_no_panics]
async fn multi_use_read_only_transaction_parallel_initial_queries_explicit_begin() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
...
#[tokio_test_no_panics]
async fn read_write_transaction_full_lifecycle_hard_affinity() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
    ...
    assert_eq!(
        batch_result.len(),
        1,
        "Expected exactly 1 batch update result"
    );
...
#[tokio_test_no_panics]
async fn read_write_transaction_parallel_initial_queries_inline_begin() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
    ...
    let (query_result, update_result) = tokio::join!(query_future, update_future);
    query_result?;
    update_result?;
...
#[tokio_test_no_panics]
async fn read_write_transaction_parallel_initial_queries_explicit_begin() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
...
#[tokio_test_no_panics]
async fn read_write_transaction_rollback_releases_rw_guard() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
...
#[tokio_test_no_panics]
async fn read_write_transaction_drop_releases_rw_guard() -> Result<()> {
    let mut mock = create_session_mock();
...
#[tokio_test_no_panics]
async fn write_only_transaction_pins_begin_and_commit_and_releases_guards() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
...
#[tokio_test_no_panics]
async fn write_only_transaction_write_at_least_once_lifecycle() -> Result<()> {
    let mut mock = create_session_mock();
    ...
    assert!(
        response.commit_timestamp.is_some(),
        "Expected commit_timestamp to be present"
    );
...
#[tokio_test_no_panics]
async fn partitioned_dml_pins_begin_and_execute_and_releases() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
...
#[tokio_test_no_panics]
async fn batch_read_only_transaction_routes_through_pool() -> Result<()> {
    let mut mock = create_session_mock();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));
...
#[tokio_test_no_panics]
async fn dynamic_channel_pool_scales_up_under_concurrent_queries() -> Result<()> {
    let mut mock = create_session_mock();
...
#[tokio_test_no_panics]
async fn static_channel_pool_remains_fixed_under_concurrent_queries() -> Result<()> {
    let mut mock = create_session_mock();
...
#[tokio_test_no_panics]
async fn p2c_avoids_channel_with_error_penalty() -> Result<()> {
    let mut mock = create_session_mock();
    let mut sequence = Sequence::new();
    let captured_addresses = Arc::new(Mutex::new(Vec::new()));

    let captured_addresses_failure = Arc::clone(&captured_addresses);
    mock.expect_execute_streaming_sql()
        .times(1)
        .in_sequence(&mut sequence)
        .returning(move |request| {
            let address = request.remote_addr().expect("remote_addr");
            captured_addresses_failure
                .lock()
                .expect("lock")
                .push(address);
    ...
    let captured_addresses_success = Arc::clone(&captured_addresses);
    mock.expect_execute_streaming_sql()
        .in_sequence(&mut sequence)
        .returning(move |request| {
            let address = request.remote_addr().expect("remote_addr");
            captured_addresses_success
                .lock()
                .expect("lock")
                .push(address);
            Ok(Response::from(adapt([Ok(setup_select1())])))
    ...
    assert!(
        database_client
            ...
            .await
            .is_err(),
        "Expected query to fail to apply penalty"
    );

    let penalized_address = {
        let addresses = captured_addresses.lock().expect("lock");
        assert_eq!(addresses.len(), 1, "First query executed");
        addresses[0]
    };
    ...
    let subsequent_on_penalized = all_addresses[1..]
        .iter()
        .filter(|address| **address == penalized_address)
        .count();

    assert_eq!(
        subsequent_on_penalized, 0,
        "P2C must completely avoid the penalized channel when an unpenalized alternative with 0 load exists"
    );
...
#[tokio_test_no_panics]
async fn zero_permit_leak_mixed_workload_battery() -> Result<()> {
    let mut mock = create_session_mock();
    ...
    let read_write_transaction = ReadWriteTransactionBuilder::new(database_client.clone())
        .build(None)
        .await?;
    let _ = read_write_transaction
        .execute_update("UPDATE Users SET x = 1")
        .await?;
    read_write_transaction.commit().await?;
    ...
    let read_write_transaction = ReadWriteTransactionBuilder::new(database_client.clone())
        ...
        .await?;
    let _ = read_write_transaction
        .execute_update("UPDATE Users SET x = 2")
        .await?;
    read_write_transaction.rollback().await?;
    ...
    let read_write_transaction = ReadWriteTransactionBuilder::new(database_client.clone())
        ...
        .await?;
    let _ = read_write_transaction
        .execute_update("UPDATE Users SET x = 3")
        .await?;
    drop(read_write_transaction);
