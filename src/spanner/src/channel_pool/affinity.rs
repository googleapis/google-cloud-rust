...
impl<'a> From<&'a Option<Arc<TransactionAffinity>>> for ChannelTarget<'a> {
    fn from(affinity: &'a Option<Arc<TransactionAffinity>>) -> Self {
        match affinity {
            Some(affinity) => Self::Affinity(affinity.as_ref()),
            None => Self::Any,
        }
    }
}
...
mod tests {
    ...
    fn channel_target_from_conversions() {
        ...
        assert!(
            matches!(target_from_ref, ChannelTarget::Affinity(a) if ptr::eq(a, &affinity)),
        ...
        assert!(
            matches!(target_from_arc, ChannelTarget::Affinity(a) if ptr::eq(a, arc_affinity.as_ref())),
            "target_from_arc affinity must match"
        );

        let target_from_some = ChannelTarget::from(Some(&affinity));
        assert!(
            matches!(target_from_some, ChannelTarget::Affinity(a) if ptr::eq(a, &affinity)),
        ...
        assert!(
            matches!(target_from_opt_arc, ChannelTarget::Affinity(a) if ptr::eq(a, arc_affinity.as_ref())),
