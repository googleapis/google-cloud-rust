...
-> anyhow::Result<()> {
    ...
    let response = database_client
        .begin_transaction(begin_request, RequestOptions::default(), ChannelTarget::Any)
        .await?;
...
async fn streaming_read_with_inline_begin_rw_records_affinity() -> anyhow::Result<()> {
    ...
    let mut stream = database_client
        .streaming_read(read_request, RequestOptions::default(), ChannelTarget::Any)
        .send()
        .await?;
...
-> anyhow::Result<()> {
    ...
    let _ = database_client
        .execute_sql(request1, RequestOptions::default(), ChannelTarget::Any)
        .await?;
    ...
    let _ = database_client
        .execute_sql(request2, RequestOptions::default(), ChannelTarget::Any)
        .await?;
...
async fn unary_rpc_feedback_unavailable_places_endpoint_on_cooldown() -> anyhow::Result<()> {
    ...
    let result = database_client
        .execute_sql(statement.into_request(), options, ChannelTarget::Any)
        .await;
    assert!(
        result.is_err(),
