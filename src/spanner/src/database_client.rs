...
macro_rules! define_db_streaming_rpc {
    ...
    ($method:ident, $expect_method:ident, $request_type:ty, $builder_type:ty, $extract_key:expr) => {
        pub(crate) fn $method<'a>(
            ...
            // - Otherwise, select via channel pool using target affinity.
            let callback =
            ...
            let builder = self
                .spanner
                .$method(request, options, lease.channel())
                .with_transaction_id_callback(callback);
            builder.with_lifetime_guard(Arc::new(lease.into_guard()))
        }
    };
...
mod tests {
    ...
    async fn resolve_routing_connection_omni_cold_start_and_cache_hit_and_cooldown() {
        ...
        //    request dispatches over the client's channel pool via channel target)
    ...
    #[tokio_test_no_panics]
    async fn transaction_affinity_gateway_fallback_preserves_channel_affinity_across_statements() {
        use std::sync::Mutex;
    ...
    #[tokio_test_no_panics]
    async fn channel_pool_affinity_routing_for_all_rpcs_when_location_routing_disabled() {
        use std::sync::Mutex;
    ...
    #[tokio_test_no_panics]
    async fn streaming_rpc_error_records_penalty_on_leased_channel() {
        use crate::channel_pool::DynamicChannelPoolConfig;
        use crate::client::SpannerBuilderExt;
        use gaxi::grpc::tonic::Status;
        use google_cloud_gax::retry_policy::NeverRetry;
        ...
        let result = database_client
            .execute_streaming_sql(request, options, None)
            .send()
            .await;
        assert!(
            result.is_err(),
            "execute_streaming_sql should fail with unavailable error"
        );

        let penalty_after_error = active_entries[0].current_penalty();
        assert_eq!(
            penalty_after_error, 15,
            "Channel entry must have error penalty applied after UNAVAILABLE streaming RPC"
        );
    }

    #[tokio_test_no_panics]
    async fn streaming_rpcs_preserve_channel_affinity_when_location_routing_enabled_without_routing_key()
     {
        use std::sync::Mutex;
