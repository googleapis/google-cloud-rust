...
impl BatchDmlBuilder {
    ...
    /// Sets whether this batch is the last statements in a read/write transaction.
    ///
    /// # Example
    /// ```
    ...
    ///     let stmt = Statement::builder("UPDATE MyTable SET MyColumn = 'MyValue' WHERE Id = 1").build();
    ///     let batch = BatchDml::builder()
    ///         .add_statement(stmt)
