        ...
        let json_data = match self.credentials_source {
            ...
            CredentialsSource::DefaultCredentials => match load_adc()? {
                AdcContents::Contents(contents) => {
                    Some(serde_json::from_str(&contents).map_err(BuildCredentialsError::parsing)?)
                }
...
fn extract_credential_type(json: &Value) -> BuildResult<&str> {
    json.get("type")
        .ok_or_else(|| BuildCredentialsError::parsing("no `type` field found."))?
        .as_str()
        .ok_or_else(|| BuildCredentialsError::parsing("`type` field is not a string."))
}
    ...
    match json {
        ...
        Some(json) => {
            ...
            match cred_type {
                ...
                _ => Err(BuildCredentialsError::unknown_type(cred_type)),
            }
        }
    }
}

fn path_not_found(path: String) -> BuildCredentialsError {
    BuildCredentialsError::loading(format!(
        "{path}. {}",
        concat!(
...
fn load_adc() -> BuildResult<AdcContents> {
    match adc_path() {
        ...
        Some(AdcPath::FromEnv(path)) => match std::fs::read_to_string(&path) {
            ...
            Err(e) => Err(BuildCredentialsError::loading(e)),
        },
