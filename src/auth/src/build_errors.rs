...
impl BuildCredentialsError {
    ...
    /// # Example
    /// ```
    /// use google_cloud_auth::BuildCredentialsError;
    /// let error = BuildCredentialsError::loading("test message");
    /// assert!(error.is_loading(), "{error:?}");
    /// ```
    pub fn loading<T>(source: T) -> BuildCredentialsError
    where
        T: Into<Box<dyn Error + Send + Sync + 'static>>,
    {
        BuildCredentialsError(BuildImpl::Loading(source.into()))
    }
    ...
    /// ```
    /// use google_cloud_auth::BuildCredentialsError;
    /// let error = BuildCredentialsError::parsing("test message");
    /// assert!(error.is_parsing(), "{error:?}");
    /// ```
    pub fn parsing<T>(source: T) -> BuildCredentialsError
    where
        T: Into<Box<dyn Error + Send + Sync + 'static>>,
    {
        BuildCredentialsError(BuildImpl::Parsing(source.into()))
    }
    ...
    /// ```
    /// use google_cloud_auth::BuildCredentialsError;
    /// let error = BuildCredentialsError::unknown_type("test message");
    /// assert!(error.is_unknown_type(), "{error:?}");
    /// ```
    pub fn unknown_type<T>(source: T) -> BuildCredentialsError
    where
        T: Into<Box<dyn Error + Send + Sync + 'static>>,
    {
        BuildCredentialsError(BuildImpl::UnknownType(source.into()))
    }
}
