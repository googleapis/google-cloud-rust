...
impl SendTask {
    ...
    const ERROR_MESSAGE_TASK_CANCELLED: &'static str = "grpc-rust request task was cancelled";

    /// Creates a new [`SendTask`] handle wrapping a background request task handle.
    ...
    ///
    /// Returns a [`SendTask`] handle for managing background task cancellation and monitoring.
    ...
    /// `true` if the background request task has not been [`join()`](Self::join)ed
    pub(super) fn is_active(&self) -> bool {
        self.handle.is_some()
    }

    /// Awaits the background request pump task and checks for send failures.
    ///
    /// Returns `None` if the request task completed successfully or was cleanly cancelled,
    /// or `Some(tonic::Status)` if writing requests to the server failed.
    pub(super) async fn join(&mut self) -> Option<tonic::Status> {
        ...
        match result {
            ...
            Err(error) => Some(tonic::Status::internal(format!(
    ...
    Request: Message + 'static,
{
    ...
    while let Some(request) = requests.next().await {
        if send
            .send(&GrpcRustSend(request), SendOptions::default())
            .await
            .is_err()
        {
            return Err(tonic::Status::internal(
                SendTask::ERROR_MESSAGE_STREAM_CLOSED,
            ));
        }
    }
    drop(send);

    // Keep the task alive after a clean close as we might still need to process server responses.
...
mod tests {
    ...
    #[tokio::test]
    async fn send_task_join_returns_none_when_task_cancelled() -> anyhow::Result<()> {
        // Arrange
        ...
        assert!(
            result.is_none(),
            "finished cancelled task should return None"
        );
        Ok(())
    }
    ...
    async fn send_task_join_returns_status_when_send_fails() -> anyhow::Result<()> {
        ...
        let status = task
            .join()
            .await
            .expect("expected status error when send fails");
    ...
    async fn send_task_join_returns_internal_status_when_task_panics() -> anyhow::Result<()> {
        ...
        let status = task
            .join()
            .await
            .expect("expected status error on task panic");
