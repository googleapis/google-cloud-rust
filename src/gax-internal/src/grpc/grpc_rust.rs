...
impl GrpcRustClient {
    ...
    async fn build(
        ...
        // TODO(#5991) [Parity gap]: `Client` applies a per-connection
        // concurrency limit of 100 in src/gax-internal/src/grpc.rs:493.
        // We can't do this here: while grpc-rust has an internal transport
        // option for this, it's not exposed in ChannelOptions.
        // Does it make sense for grpc-rust to support this?

        // TODO(#5991) [Parity gap]: The existing Tonic-based `Client` uses
        ...
        // exposed for use.
        //
        // For now, use grpc-rust's default, `pick_first`, and conduct
        // measurements. We will also ask if grpc-rust can expose a way to use
        // `round_robin` and set its subchannel count.
        //
        // An alternative would be to manually implement an `InvokerPool`
        // (sketched out in the design doc) that performs round-robin load balancing and
        // respects grpc_subchannel_count. Such an implementation would
        // duplicate DNS/LB state across Channels (unlike Tonic).

        // TODO(#5991) [Parity gap]: grpc-rust only uses a single channel
        // authority for both the HTTP/2 `:authority` and TLS SNI.
        // (In Tonic these can be different values.). Separate values
        // are needed for global and VPC-SC private/restricted VIPs and
        // Private Service Connect. See `crate::host::origin` and its tests for
        // examples.
        //
        // Until grpc-rust supports separate TLS server-name and
        // request-authority settings, use the request origin as the shared
        // authority.
...
fn resolve_transport_endpoint(
    ...
    let endpoint = ::tonic::transport::Endpoint::from_shared(target_endpoint.clone())
        .map_err(BuilderError::transport)?;
    let target_uri = endpoint.uri();
...
fn resolve_port(uri: &Uri, authority: &http::uri::Authority) -> Option<u16> {
    authority.port_u16().or_else(|| match uri.scheme() {
        Some(scheme) if scheme == &http::uri::Scheme::HTTPS => Some(443),
        Some(scheme) if scheme == &http::uri::Scheme::HTTP => Some(80),
        _ => None,
    })
}
...
fn make_tracing_attributes(
    ...
    Ok(super::TracingAttributes {
        ...
        url_domain: default_uri.host().unwrap_or("").to_string(),
        instrumentation,
    })
}

// TODO(#5991): [Parity gap]: Tonic first uses an installed process-default
// CryptoProvider; otherwise, it selects the provider enabled through its Cargo
// features. In contrast, grpc-rust's RustlsChannelCredendials::new requires a
// process-wide rustls CryptoProvider to be installed.
//
// Can grpc-rust either mimic Tonic's behaviour or provide a way to inject a
// CryptoProvider?
//
// For now, we install aws-lc-rs if the _default-rustls-provider feature is
// enabled; otherwise, we raise an error if the user hasn't installed a
// process-wide one.
//
// Alternatives are:
//
// 1. Require every user to install a process default. This avoids our direct
//    rustls dependency, but makes the SDK's default TLS configuration fail
//    unless the user does the setup.
// 2. Attempt to install aws-lc-rs unconditionally (i.e. remove the feature
//    gating here). This is simple, but prevents users from choosing their
//    provider.
// 3. Implement custom grpc-rust channel credentials. This is a fair bit of work.
