...
impl ErrorType {
    ...
    pub(crate) fn as_str(&self) -> String {
        match self {
            ...
            ErrorType::GrpcError { code, .. } => code.name().to_string(),
    ...
    pub(crate) fn grpc_code(&self) -> Code {
        match self {
            ...
            ErrorType::ClientAuthenticationError => Code::Unauthenticated,
    ...
    pub(crate) fn http_code(&self) -> Option<u16> {
        match self {
            ErrorType::HttpError { code, .. } => Some(code.as_u16()),
            _ => None,
        }
    }
}
...
pub(crate) mod tests {
    ...
    fn http_error(http_code: StatusCode, rpc_code: Code) -> (ErrorType, String, Code, Option<u16>) {
        (
            ErrorType::HttpError {
                code: http_code,
                reason: None,
            ...
            rpc_code,
            Some(http_code.as_u16()),
        )
    }

    fn grpc_error(grpc_code: Code) -> (ErrorType, String, Code, Option<u16>) {
        (
            ErrorType::GrpcError {
                code: grpc_code,
                reason: None,
            },
            grpc_code.name().to_string(),
            grpc_code,
            None,
        )
    }
    ...
    fn client_error(
        ...
        name: &str,
        code: Code,
    ) -> (ErrorType, String, Code, Option<u16>) {
        (error_type, name.to_string(), code, None)
    }

    #[test]
    fn test_error_type_conversions() {
        // Table of test case tuples.
        // (error_type: ErrorType, expected_as_str: String, expected_grpc_code: Code, expected_http_code: Option<u16>)
        let test_cases = vec![
        ...
        for (error_type, expected_as_str, expected_grpc_code, expected_http_code) in test_cases {
            assert_eq!(
                error_type.as_str(),
                expected_as_str,
                "expected as_str for {:?}",
