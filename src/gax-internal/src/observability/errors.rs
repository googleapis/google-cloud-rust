...
impl ErrorType {
    pub(crate) fn from_gax_error(err: &gax::error::Error) -> Self {
        match err {
            ...
            e if e.is_exhausted() => ErrorType::ResourceExhausted,
            ...
            e if e.is_io() || e.is_connect() => Self::from_io_error(e),
            e => {
    ...
    fn from_io_error(err: &gax::error::Error) -> Self {
        // Attempt to detect redirect error from inner reqwest error
        let mut source = err.source();
        while let Some(s) = source {
            if let Some(reqwest_err) = s.downcast_ref::<reqwest::Error>() {
                if reqwest_err.is_redirect() {
                    return ErrorType::ClientRedirectError;
                }
            }
            source = s.source();
        }
...
pub(crate) mod tests {
    ...
    #[test_case(ErrorType::ResourceExhausted, RESOURCE_EXHAUSTED; "Resource Exhausted")]
    ...
    #[test_case(Error::exhausted("test"), RESOURCE_EXHAUSTED; "Exhausted")]
