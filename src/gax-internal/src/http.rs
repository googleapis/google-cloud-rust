...
impl ReqwestClient {
    ...
    async fn request_attempt<O: serde::de::DeserializeOwned + Default>(
        ...
        #[cfg(google_cloud_unstable_tracing)]
        let status_code = reqwest_result.as_ref().ok().map(|r| r.status());

        let gax_result: Result<Response<O>> = match reqwest_result {
            Ok(response) => {
        ...
        if let Some(ref s) = span {
            let _enter = s.enter(); // Re-enter span context
            record_http_response_attributes(s, &gax_result, status_code);
        }
