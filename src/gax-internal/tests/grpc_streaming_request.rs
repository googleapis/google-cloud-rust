...
mod tests {
    ...
    #[cfg(google_cloud_unstable_gapic_streaming)]
    #[tokio::test]
    async fn execute_bidi_streaming_drop_receiver_closes_sender() -> anyhow::Result<()> {
        let (endpoint, _server) = start_echo_server().await?;
        let client = builder(endpoint)
            .with_credentials(test_credentials())
            .build()
        ...
        // Dropping the receiver aborts connection/stream setup in the background task.
        drop(receiver);

        // Sending should fail with StreamClosed once the background task cancels and drops req_stream.
        let mut err = None;
        for _ in 0..20 {
        ...
        assert!(matches!(
            err,
            Some(google_cloud_gax::streaming::SendError::StreamClosed)
        ));
