...
#[derive(Clone, Debug)]
pub struct Row {
    pub(crate) values: Vec<crate::query::from_sql::SqlValueInner>,
...
impl Row {
    ...
    fn convert_value_at<T: FromSql>(
        &self,
        idx: usize,
        val: crate::query::from_sql::SqlValueInner,
    ) -> Result<T> {
    ...
    pub fn take<T: FromSql, I: ColumnIndex>(&mut self, index: I) -> Result<T> {
        ...
        let owned_val = std::mem::replace(val, crate::query::from_sql::SqlValueInner::Null);
...
) -> Result<Vec<crate::query::from_sql::SqlValueInner>> {
...
) -> Result<crate::query::from_sql::SqlValueInner> {
    use crate::query::from_sql::SqlValueInner;
    match value {
...
fn convert_repeated(
    ...
    Ok(crate::query::from_sql::SqlValueInner::Array(arr))
}
...
fn convert_nested(
    ...
    let entries = fields
        ...
        .collect();
    Ok(crate::query::from_sql::SqlValueInner::Struct(entries))
}

fn convert_basic_type(
    value: String,
    field_name: &str,
    field_type: &str,
) -> Result<crate::query::from_sql::SqlValueInner> {
...
mod tests {
    ...
    #[derive(crate::query::FromSql, Debug, PartialEq)]
    struct AnonTupleStruct(i64, String, bool);
    ...
    async fn anonymous_struct_preserves_all_fields() -> TestResult {
        ...
        let anon: AnonTupleStruct = row.take("anon")?;
        assert_eq!(anon, AnonTupleStruct(10, "hello".to_string(), true));
        Ok(())
    }
    ...
    async fn struct_preserves_sql_declaration_order_over_alphabetical_order() -> TestResult {
        ...
        let by_pos: TupleZThenA = row.get("pair")?;
        assert_eq!(by_pos, TupleZThenA(1, 2));
        Ok(())
    }
    ...
    async fn duplicate_struct_field_names_match_row_behavior() -> TestResult {
        ...
        let both_ids: DupIdTuple = row.get("dup")?;
        assert_eq!(both_ids, DupIdTuple(100, 200));
        Ok(())
    }
